...
from phodcos.utils.ph_curve import Bezier
from phodcos.utils.parameterization import erf_continuity, preimage_continuity
...
class PHODCOS:
    ...
    __OCC_MAX_POINTS = 100000  # maximum number of points in occupancy map
    __HP_MAX_PLANES = 20  # maximum number of hyperplanes in polygon

    __TOL_PROJECT = 5e-1  # Distance below which a point is projected onto corridor's section cut  # high: smaller / more consistent corridors

    def __init__(self, interp_order, n_segments, n_eval):
        self._interp_order = interp_order
        self._n_segments = n_segments
        self._n_eval = n_eval

        self._nominal_path = None
        self._parametric_path = None
        self._corridor = None
        self._error = None

        self._parametric_functions = None
        self._parametric_segment_functions = None
        self._set_parametric_functions()
    ...
    @property
    def nominal_path(self):
        return self._nominal_path

    @property
    def parametric_path(self):
        return self._parametric_path
    ...
    @property
    def parametric_functions(self):
        return self._parametric_functions

    @property
    def parametric_segment_functions(self):
        return self._parametric_segment_functions

    @interp_order.setter
    def interp_order(self, new_interp_order):
        if new_interp_order not in [2, 4]:
            raise ValueError("Interpolation order must be 2 or 4")
        else:
            self._interp_order = new_interp_order
    ...
    @n_eval.setter
    def n_eval(self, new_n_eval):
        self._n_eval = new_n_eval

    def parameterize_path(self, nominal_path):
        interp_order = self._interp_order
        n_segments = self._n_segments
        parametric_funcs = self._parametric_segment_functions

        xi_grid = np.linspace(0, 1, n_segments + 1)
        ...
        self._nominal_path = nominal_path
        self._parametric_path = {
            "A0": A0_rot,
            "A1": A1_rot,
            "A2": A2_rot,
            "A3": A3_rot,
    ...
    def evaluate_parameterization(self):
        interp_order = self._interp_order
        n_eval = self._n_eval * self._n_segments
        parametric_funcs = self._parametric_functions
        parametric_path = self._parametric_path
        ...
        self._nominal_path = nominal_path
        self._parametric_path = parametric_path
        self._error = error

    def evaluate_corridor(self, occ):
        n_eval = self._n_eval * self._n_segments
        interp_order = self._interp_order
        parametric_funcs = self._parametric_functions
        parametric_path = self._parametric_path

        pa_d = np.zeros((n_eval, 2))
        pa_Acr = np.zeros((n_eval, self.__HP_MAX_PLANES, 2))
        pa_bcr = np.zeros((n_eval, self.__HP_MAX_PLANES, 1))
        pa_occ = []  # np.zeros((n_eval, self.__OCC_MAX_POINTS, 2))

        Dim = occ.shape[1]  # 2 (planar) or 3 (spatial)

        # add remaining (virtual) occupancy points
        occ = np.vstack(
            (occ, 1e10 * np.ones((self.__OCC_MAX_POINTS - occ.shape[0], Dim)))
        )
        ...
        for k, xik in enumerate(xi_eval):
            if interp_order == 2:
                if Dim == 2:
                    d_pos, d_neg = parametric_funcs["corridor_2d"](
                        xik,
                        parametric_path["xi_grid"],
                        parametric_path["translation"],
                        parametric_path["A0"],
                    ...
                    Acr, bcr, n_hp, occ_erf = parametric_funcs["corridor_3d"](
                        xik,
                        parametric_path["xi_grid"],
                        parametric_path["translation"],
                        parametric_path["A0"],
            ...
            if Dim == 2:
                pa_d[k, :] = np.array([d_pos.full().item(), d_neg.full().item()])

            elif Dim == 3:
                occ_erf = np.squeeze(occ_erf)

                pa_Acr[k, :, :] = np.squeeze(Acr)
                pa_bcr[k, :, :] = np.squeeze(bcr)[:, None]
                pa_occ += [occ_erf[np.linalg.norm(occ_erf, axis=1) < 1e2]]

        # pa_occ = np.squeeze(pa_occ)
        if Dim == 2:
            self._corridor = {"d": pa_d}
        elif Dim == 3:
            self._corridor = {"A": pa_Acr, "b": pa_bcr, "occ_erf": pa_occ, "occ": occ}

    def compute_controlpoints(self):
        """Computes the quaternion (1) polynomial, (2) hodograph and (3) position
        controlpoints for their respective binomial expression.

        NOTE: This function can only be called AFTER parameterization

        Returns:
            dict: {"A","h","p"}
        """
        interp_order = self.interp_order
        parametric_path = self.parametric_path
        n_segments = self.n_segments
        f_h = self._parametric_segment_functions["h_ctrlpts"]
        f_p = self._parametric_segment_functions["p_ctrlpts"]

        # initialize variables
        t = parametric_path["translation"]
        A = [
            parametric_path["A0"],
            parametric_path["A1"],
            parametric_path["A2"],
        ...
        for k in range(n_segments):
            if interp_order == 2:
                hodograph = f_h(A[0][k], A[1][k], A[2][k], A[3][k], A[4][k])
                points = f_p(t[k], A[0][k], A[1][k], A[2][k], A[3][k], A[4][k])
            elif interp_order == 4:
                hodograph = f_h(
                    A[0][k],
                    A[1][k],
    ...
    def visualize_parameterization(self):
        parametric_path = self.parametric_path
        ...
        ax.plot(nominal_path["xi"], nominal_path["a"] ** 2, "k--")
        ax.set_ylabel("a")
    ...
    def _set_parametric_functions(self):
        if self.__VERBOSE:
            print(
                "Generating parametric functions for", self._n_segments, "segments, ..."
            )
        self._parametric_segment_functions = self._set_parametric_segment_functions()
        self._parametric_functions = self._set_parametric_curve_functions()
        if self.__VERBOSE:
            print("Done")

    def _set_parametric_segment_functions(self):
        ...
        if interp_order == 2:
            ...
            f_h = cs.Function("f_h", [A0, A1, A2, A3, A4], hodograph)
            ...
            h13 = (
                1
                / -3360
                * (cs.vertcat(0, se) - (10080 * h14 - 10080 * h15 + 3360 * h16))
            )
            ...
            f_h = cs.Function("f_h", [A0, A1, A2, A3, A4, A5, A6, A7, A8], hodograph)
            f_p = cs.Function(
                "f_p",
    ...
    def _set_parametric_curve_functions(self):
        """Generates symbolic functions for C2 or C4 hermite interpolation of PH curves
        interp_order: Order of hermite interpolation (2 or 4)
        ...
        """
        interp_order = self._interp_order
        n_segments = self._n_segments  # 2**n

        parametric_segment_funcs = self._parametric_segment_functions

        occ_2d = cs.MX.sym("occ", self.__OCC_MAX_POINTS, 2)
        occ_3d = cs.MX.sym("occ", self.__OCC_MAX_POINTS, 3)
